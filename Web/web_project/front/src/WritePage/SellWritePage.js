...
export default function SellWritePage() {
  ...
  const uploadImgFile = async (e) => {
    ...
    if (state.imgFile.length + files.length > 10) {
  ...
  const handlePostSubmit = async () => {
    if (!state.productName || !productPrice || !state.productContent || !state.productType || !state.selectedCategory || state.postImg.length === 0) {
